...
function reduceBgMusicVolume() {
  bgMusic.volume = originalBgVolume * 0.1;
}
...
function playGame(playerSelection, enemySelection) {
  if (playerSelection === enemySelection) {
    ...
    (enemySelection === "LANCE" && playerSelection === "SWORD") ||
...
themeChooser.addEventListener("click", (event) => {
  ...
  const blueFirst = getComputedStyle(document.body).getPropertyValue(
    "--blue-first"
  );
  const blueSecond = getComputedStyle(document.body).getPropertyValue(
    "--blue-second"
  );
  const redFirst = getComputedStyle(document.body).getPropertyValue(
    "--red-first"
  );
  const redSecond = getComputedStyle(document.body).getPropertyValue(
    "--red-second"
  );
  const yellowFirst = getComputedStyle(document.body).getPropertyValue(
    "--yellow-first"
  );
  const yellowSecond = getComputedStyle(document.body).getPropertyValue(
    "--yellow-second"
  );

  const blueHeaderBottom = getComputedStyle(document.body).getPropertyValue(
    "--blue-header-bottom"
  );
  const redHeaderBottom = getComputedStyle(document.body).getPropertyValue(
    "--red-header-bottom"
  );
  const yellowHeaderBottom = getComputedStyle(document.body).getPropertyValue(
    "--yellow-header-bottom"
  );

  const root = document.documentElement;

  const modalBlue = getComputedStyle(document.body).getPropertyValue(
    "--modal-blue"
  );
  const modalRed = getComputedStyle(document.body).getPropertyValue(
    "--modal-red"
  );
  const modalYellow = getComputedStyle(document.body).getPropertyValue(
    "--modal-yellow"
  );
